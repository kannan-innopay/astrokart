    ...
    <div class="min-h-screen bg-gradient-to-br from-night-950 via-night-900 to-cosmic-950 px-4 py-10">
        ...
        <div class="mx-auto w-full max-w-2xl animate-fade-up">
            <div class="mb-6 text-center">
                <h1 class="font-display text-2xl font-bold text-white">Astrologer Application</h1>
                <p class="mt-1 text-sm text-cosmic-200">Tell us about yourself. Your profile goes live after admin verification.</p>
            </div>
            ...
            <form method="POST"
                ...
                <div class="rounded-2xl border border-white/10 bg-white p-6 shadow-2xl shadow-cosmic-950/50">
                    ...
                    <div x-show="step === 1" class="space-y-4">
                        <h2 class="text-base font-semibold text-gray-900">Professional details</h2>

                        <x-input name="name" label="Full name" :value="old('name')" required />
                        <x-textarea name="bio" label="Short bio" :rows="3" placeholder="Your background, approach and specialisations...">{{ old('bio') }}</x-textarea>

                        <div class="grid grid-cols-2 gap-4">
                            <x-input name="years_of_experience" type="number" label="Years of experience" :value="old('years_of_experience')" min="0" max="100" required />
                            <x-input name="price_per_minute" type="number" label="Price / min (₹)" :value="old('price_per_minute', 5)" min="5" required />
                        </div>

                        <div>
                            <span class="mb-1.5 block text-sm font-medium text-gray-700">Consultation modes</span>
                            <div class="flex flex-wrap gap-2">
                                @php($modes = ['chat' => 'Chat', 'call' => 'Voice call', 'video_call' => 'Video call'])
                    ...
                    <div x-show="step === 2" x-cloak class="space-y-4">
                        <h2 class="text-base font-semibold text-gray-900">Profile photos</h2>
                        <p class="text-sm text-gray-500">Add at least one clear photo. The first one becomes your primary profile picture. You can add up to <span x-text="maxPhotos"></span>.</p>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <label :class="files.length >= maxPhotos && 'pointer-events-none opacity-50'"
                                ...
                                <span class="text-sm font-medium text-cosmic-700">Choose from gallery</span>
                            ...
                            <button type="button" @click="takePhoto()" :disabled="files.length >= maxPhotos"
                                ...
                                <span class="text-sm font-medium text-cosmic-700">Take a photo</span>
                            </button>
                        ...
                        <p class="text-xs text-gray-400" x-show="files.length"><span x-text="files.length"></span> / <span x-text="maxPhotos"></span> photos added</p>
                        ...
                        <div class="grid grid-cols-3 gap-3" x-show="previews.length">
                            <template x-for="(src, i) in previews" :key="src">
                                <div class="relative">
                                    ...
                                    <span x-show="i === 0" class="absolute left-1 top-1 rounded bg-gold-500 px-1.5 py-0.5 text-[10px] font-semibold text-night-950">Primary</span>
                        ...
                        <div x-show="camera.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" @keydown.escape.window="closeCamera()">
                            <div class="w-full max-w-md rounded-2xl bg-white p-4 shadow-2xl">
                                ...
                                <div class="mt-4 flex items-center justify-between gap-3">
                                    <button type="button" @click="closeCamera()" class="rounded-xl px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-100">Cancel</button>
                    ...
                    <div x-show="step === 3" x-cloak class="space-y-4">
                        <h2 class="text-base font-semibold text-gray-900">Identity verification <span class="text-sm font-normal text-gray-400">(optional)</span></h2>
                        <div class="rounded-lg bg-gold-50 px-4 py-3 text-sm text-gold-800">
                            ✨ Verified astrologers get a trust badge and appear higher in search — leading to more customers.
                        </div>

                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-input name="aadhaar_number" label="Aadhaar number" :value="old('aadhaar_number')" inputmode="numeric" maxlength="12" placeholder="12-digit number" />
                            <div>
                                <label for="aadhaar_file" class="mb-1.5 block text-sm font-medium text-gray-700">Aadhaar document</label>
                            ...
                            <x-input name="pan_number" label="PAN number" :value="old('pan_number')" maxlength="10" placeholder="ABCDE1234F" class="uppercase" />
                            <div>
                                <label for="pan_file" class="mb-1.5 block text-sm font-medium text-gray-700">PAN document</label>
                        ...
                        <div>
                            <label for="certificate_file" class="mb-1.5 block text-sm font-medium text-gray-700">Qualification certificate</label>
                    ...
                    <div x-show="step === 4" x-cloak class="space-y-4">
                        <h2 class="text-base font-semibold text-gray-900">Settlement details <span class="text-sm font-normal text-gray-400">(optional)</span></h2>
                        <p class="text-sm text-gray-500">Where we send your consultation earnings. You can also add these later from your dashboard.</p>

                        <x-input name="bank_account_name" label="Account holder name" :value="old('bank_account_name')" />
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-input name="bank_account_number" label="Account number" :value="old('bank_account_number')" />
                            <x-input name="bank_ifsc_code" label="IFSC code" :value="old('bank_ifsc_code')" maxlength="11" placeholder="HDFC0001234" class="uppercase" />
                        </div>
                        <x-input name="upi_id" label="UPI ID" :value="old('upi_id')" placeholder="name@bank" />
                    </div>
                ...
                return {
                    ...
                    async openCamera() {
                        ...
                        try {
                            ...
                            this.camera.error = e?.name === 'NotAllowedError'
